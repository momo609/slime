...
class Eagle3TrainingManager:
    ...
    def initialize(
        ...
        os.environ.setdefault("SLIME_SGLANG_DRAFTER_RETURN_LAST_HIDDEN", "1")
    ...
    def collect_rollout_data(self, rollout_data, hidden_states=None, target_logprobs=None):
        ...
        Args:
            ...
            target_logprobs: Optional dense logprobs [seq_len, vocab_size] from rollout
        """
        if not self.is_initialized or not self.eagle3_trainer:
            return
        self.eagle3_trainer.collect_online_data(rollout_data, hidden_states, target_logprobs)
    ...
    async def train_drafter(self, rollout_id: int):
        ...
        success = await self.eagle3_trainer.training_step(rollout_id)
        
        if success:
            logger.info(f"Eagle3 training step {rollout_id} completed")
        else:
            logger.warning(f"Eagle3 training step {rollout_id} failed")
