...
class RolloutManager:
    ...
    def _convert_samples_to_train_data(self, samples: list[Sample] | list[list[Sample]]):
        ...
        """
        # print("######################self.custom_convert_samples_to_train_data_func############", self.custom_convert_samples_to_train_data_func)
        ...
        # print("######################samples[0].hidden_states  is not None############", samples[0].hidden_states is not None)
        if samples[0].hidden_states is not None:
            # print("######################samples[0].hidden_states############")
            train_data["hidden_states"] = [sample.hidden_states for sample in samples]

        if samples[0].target_scores is not None:
            # print("######################samples[0].hidden_states############")
            train_data["target_scores"] = [sample.target_scores for sample in samples]
    ...
    def _split_train_data_by_dp(self, data, dp_size):
        ...
        for i in range(dp_size):
            ...
            for key in [
                ...
                "target_scores",  # Add hidden states for Eagle3
                "prompt",
                "teacher_log_probs",
            ]:
                if key not in data:
