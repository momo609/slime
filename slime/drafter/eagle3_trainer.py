...
class DataBuffer:
    ...
    def add_batch(self, batch: dict, hidden_states: list = None):
        ...
        print("########add_batch########batch_size",batch_size)
        for i in range(batch_size):
            item = {
                "input_ids": batch["tokens"][i] if isinstance(batch["tokens"], list) else batch["tokens"][i].cpu(),
                "step": self.current_step,
                "target_scores": batch["target_scores"][i] if isinstance(batch["target_scores"], list) else batch["target_scores"][i].cpu(),
            }
...
class Eagle3BackgroundTrainer:
    ...
    def _save_checkpoint_async(self, step: int, is_final: bool = False):
        ...
        print("##################self.checkpoint_dir", step)
        if not self.checkpoint_dir:
            return None
        
        try:
            print("##################_save_checkpoint_async", step)
            ...
            # Save other state (optimizer, step) using torch.save
            other_state = {
                "optimizer": optimizer_state_dict,
                "step": step,
            }
            torch.save(other_state, os.path.join(checkpoint_path, "other_state.pt"))
    ...
    def collect_online_data(self, batch: dict, hidden_states: list = None):
        ...
        # Add to both local deque and data buffer
        ...
        for i in range(batch_size):
            ...
            if h_state is not None:
                ...
                # logger.exception(
                #     f"collect_online_data tokens 299 {self.collected_data}",
                # )
            
    def _prepare_training_batch(self, use_buffer_data: bool = True, buffer_steps: int = 1):
        ...
        for item in items:
            ...
            print("##########_prepare_training_batch###item",item.keys())
            ...
            rollout_log_probs = item["target_scores"].to(dev, torch.bfloat16, non_blocking=True)
            print("##########_prepare_training_batch###rollout_log_probs",rollout_log_probs.shape)
            ...
            # Apply same window slicing to rollout_log_probs [1, seq_len, vocab_size]
            r_seq_len = rollout_log_probs.size(1)
            r_vocab = rollout_log_probs.size(-1)
            if r_seq_len < window_len:
                r_pad = torch.zeros(1, window_len - r_seq_len, r_vocab, dtype=rollout_log_probs.dtype, device=dev)
                seq_rollout_log_probs = torch.cat([rollout_log_probs, r_pad], dim=1)
            elif r_seq_len > window_len:
                if start < r_seq_len:
                    r_actual_end = min(r_seq_len, start + window_len)
                    seq_rollout_log_probs = rollout_log_probs[:, start:r_actual_end]
                    if seq_rollout_log_probs.size(1) < window_len:
                        r_pad = torch.zeros(1, window_len - seq_rollout_log_probs.size(1), r_vocab,
                                            dtype=rollout_log_probs.dtype, device=dev)
                        seq_rollout_log_probs = torch.cat([seq_rollout_log_probs, r_pad], dim=1)
                else:
                    seq_rollout_log_probs = rollout_log_probs[:, -window_len:]
            else:
                seq_rollout_log_probs = rollout_log_probs

            input_ids_list.append(seq_input_ids)
            loss_mask_list.append(seq_loss_mask)
            hidden_states_list.append(seq_hidden_states)
            rollout_log_probs_list.append(seq_rollout_log_probs)
        ...
        rollout_log_probs_concat = torch.cat(rollout_log_probs_list, dim=1)
        ...
        return {
            ...
            "target": target,
            "loss_mask": loss_mask,
            "rollout_log_probs": rollout_log_probs_concat,
        }
    ...
    async def _training_step_impl(self, step: int) -> bool:
        ...
        if self.training_steps % 10 == 0:
            logger.info(
                ...
                # f"vloss={float(vloss.item()):.4f}, ploss={float(ploss.item()):.4f}"
            )
        self._save_checkpoint_async(0, is_final=False)
