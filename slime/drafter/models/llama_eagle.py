...
def compute_accuracy(
    ...
    target_tokens = targets.argmax(dim=-1)
...
def loss_function(
    ...
    finite_target_mask = torch.isfinite(targets).any(dim=-1)

    finite_scores = torch.isfinite(targets)
    finite_floor = torch.finfo(targets.dtype).min
    safe_targets = torch.where(
        finite_scores,
        targets,
        torch.full_like(targets, finite_floor),
    )
    safe_targets = torch.where(
        finite_target_mask.unsqueeze(-1),
        safe_targets,
        torch.zeros_like(safe_targets),
    )
    target_p = F.softmax(safe_targets, dim=-1)
    ...
    per_token_ploss = -(target_p * log_probs).sum(dim=-1)
    valid_position = finite_target_mask & finite_logits

    if loss_mask is not None:
        position_mask = valid_position.float() * loss_mask
        per_token_ploss = per_token_ploss * position_mask
        denominator = position_mask.sum(dim=1) + 1e-5
    else:
        denominator = logits.shape[1]

    batch_loss = torch.sum(per_token_ploss) / denominator
...
def compute_metrics(
    ...
    s_full_acc = compute_accuracy(s_logits, s_targets, s_loss_mask)
...
class LlamaForCausalLMEagle3(nn.Module):
    ...
    def forward(
        ...
        if hidden_states.size(-1) == self.target_hidden_size * 4:
            last_hidden_states = hidden_states[..., self.target_hidden_size * 3:]
            hidden_states = hidden_states[..., :self.target_hidden_size * 3]
        else:
        ...
        with torch.no_grad():
            if target_logprobs is not None:
                tg = target_logprobs.to(device=device, dtype=torch.float32)
                if tg.dim() == 2:
                    tg = tg.unsqueeze(0)
                targets = tg[..., self.t2d]
            elif last_hidden_states is not None:
                targets = self.verifier_lm_head(
                    self.verifier_norm(last_hidden_states)
                )
            else:
                targets = self.verifier_lm_head(
                    self.verifier_norm(current_hidden_states)
                )
        ...
        for idx in range(ttt_length):
            ...
            if not is_last:
                input_ids = self._shift_right(input_ids)
                loss_mask = self._shift_right(loss_mask)
